...
public class SmgGun1 : MonoBehaviour
{
    ...
    public void Reload () {
        if (reloading == false && totalBullets > 0 && originalMagazineSize - magazineSize != 0) {
            ...
            anim.SetBool("SmgGun1-R", true);
            reloading = true;
            Invoke ("CheckReload", 1.5f);
            StopShooting();
    ...
    void FixedUpdate()
    {
        ...
        if (adsing == false && shooting == false) {
            ...
            if (comeBack == false) {
                if (bloomNum <= 125) {
                    bloomNum = bloomNum + 300 * Time.deltaTime;
                } else {
                    comeBack = true;
                }
            }
        ...
        if (shooting == true) {
            ...
            if (addKickBack == false) {
                numKickBack = numKickBack - Time.deltaTime * 30;
            } else {
                numKickBack = numKickBack + Time.deltaTime * 30;
            }
        } else  {
        ...
        if (Physics.Raycast(fpsCam.transform.position, bloom, out hit)){
            ...
            if (target != null) {
                if (hit.transform.tag == "Enemy") {
                        ...
                        target.TakeDamage((int)((damage - 2) * gun.damageMultiplier));
                        stats.damageDealt += (int)((damage - 2) * gun.damageMultiplier);
                    } else {
