...
public class PistolGun1 : MonoBehaviour
{
    public GunManager gun;
    public Transform gunAnimPosition;
    ...
    public Vector3 fakeStartAds;
    public Transform realGunPos;
    ...
    void Start () {
        ...
        fakeStartAds = new Vector3 (startAds.localPosition.x, startAds.localPosition.y, startAds.localPosition.z);
    ...
    public void Reload () {
        if (reloading == false && totalBullets > 0 && originalMagazineSize - magazineSize != 0) {
            ...
            anim.SetBool("PistolGun1-R", true);
            reloading = true;
    ...
    void FixedUpdate()
    {
        ...
        if (shooting == true) {
            ...
            if (addKickBack == false) {
                numKickBack = numKickBack - Time.deltaTime * 15;
            } else {
                numKickBack = numKickBack + Time.deltaTime * 15;
            }
        } else  {
    ...
    public void Ads () {
        ...
        anim.SetBool("PistolGun1-R", false);
        ...
        if (Physics.Raycast(fpsCam.transform.position, bloom, out hit)){
            ...
            if (target != null) {
                if (hit.transform.tag == "Enemy") {
                        if (hit.distance > 100) {
                        target.TakeDamage((int)((damage - 8) * gun.damageMultiplier));
                        stats.damageDealt += (int)((damage - 8) * gun.damageMultiplier);
                    } else if (hit.distance > 75) {
                        target.TakeDamage((int)((damage - 6) * gun.damageMultiplier));
                        stats.damageDealt += (int)((damage - 6) * gun.damageMultiplier);
                    } else if (hit.distance > 35){
                        target.TakeDamage((int)((damage - 4) * gun.damageMultiplier));
                        stats.damageDealt += (int)((damage - 4) * gun.damageMultiplier);
                    } else {
