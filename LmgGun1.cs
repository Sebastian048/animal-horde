...
public class LmgGun1 : MonoBehaviour
{
    ...
    public void Reload () {
        if (reloading == false && totalBullets > 0 && originalMagazineSize - magazineSize != 0) {
            ...
            anim.SetBool("LmgGun1-R", true);
            reloading = true;
            Invoke ("CheckReload", 4f);
            StopShooting();
        ...
        if (Physics.Raycast(fpsCam.transform.position, bloom, out hit)){
            ...
            if (target != null) {
                if (hit.transform.tag == "Enemy") {
                        if (hit.distance > 100) {
                        target.TakeDamage((int)((damage - 12) * gun.damageMultiplier));
                        stats.damageDealt += (int)((damage - 12) * gun.damageMultiplier);
